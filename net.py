...
images_path = './example/*.png'
output_image_path = './eyex4rgba'
output_mat_path = './eyex4mat'
...
lr_images = []
kernels = []
path0 = glob.glob(images_path)
path0 = sorted(path0, key = os.path.getctime)
...
#     plt.show()
    ...
    savemat('%s/%d_X4_%d.mat' % (output_mat_path, i+1, 0), {'kernel': kernel})
    plt.imsave('%s/%d_X4.png' % (output_image_path, i+1), lr, vmin=0, vmax=1)
